...
int main(int argc, char **argv) {
    if (argc >= 2 && strcmp(argv[1], "-c") == 0) {
         ...
         * issues). cd/PATH/PS1/HOME/HISTFILE all come from the rc file
         * generated by RootProvisioner (see docs/DESIGN-SPEC.md, Этап 5) -
         * sourced via $ENV for plain sh, or explicitly via --rcfile for bash.
         *
         ...
         * and is a prerequisite for tmux, which several sessions want. Falls
         * back to plain sh if bash isn't installed - nothing breaks either
         * way, this is purely a nicer-if-available upgrade. */
        char bash_path[512];
        snprintf(bash_path, sizeof(bash_path), "%s/bash", SSHD4A_BIN_DIR);

        struct stat st;
        if (stat(bash_path, &st) == 0) {
            debug_log("interactive-mode, bash found, about to exec bare su -> bash --rcfile");
            execlp("su", "su", SSHD4A_TARGET_UID_STR, bash_path,
                   "--rcfile", SSHD4A_RC_FILE, "-i", (char *) NULL);
